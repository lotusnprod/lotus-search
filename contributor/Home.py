...
st.set_page_config(
    page_title="Lotus admin - contributions",
    page_icon=":lotus:",
    layout="wide",
...
def load_files_into_db():
    for file in CONTRIBUTIONS_PATH.glob("*.json"):
        ...
        with conn.session as s:
            query = text(
                "SELECT COUNT(*) FROM single_contribution WHERE filename = :filename"
            )
            ...
            if count == 0:
                ...
                s.execute(
                    query,
                    {
                        ...
                        "selected_organism": content["selected_organism"],
...
singles_df["structure"] = singles_df["molecule"].apply(
    lambda x: "data:image/svg+xml;utf8," + molecule_svg(x)
)
edited_df = st.data_editor(
    ...
    num_rows="dynamic",
    column_config={
        "structure": st.column_config.ImageColumn("Structure", help="Structure"),
        "molecule": st.column_config.TextColumn("Molecule", help="Molecule", width=50),
