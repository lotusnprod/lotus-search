...
def generate_structures_cards(active_page: int, data: dict[str, Any], molecule: str | None = None) -> list[dbc.Card]:
    cards = []
    ...
    displayed = data["matching_structures"][PAGE_SIZE * (active_page - 1) : PAGE_SIZE * active_page]
    ...
    for idx, j in enumerate(displayed):
        ...
        if scores_mode:
            ...
            extras.append(dbc.Row(dbc.Col(dbc.Progress(key=f"score_{idx}", value=int(scores[idx] * 100), max=100.0))))
        ...
        card = dbc.Card(
            [
                ...
                dbc.CardBody([
                    ...
                    dcc.Markdown(f"[Wikidata page of structure](https://www.wikidata.org/entity/Q{j})"),
                    dbc.Button("structure page", color="primary", href=f"/structure/{j}"),
                    *extras,
                ]),
            ],
            style={"width": "18rem"},
