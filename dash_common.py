...
cache = Cache(get_app().server, config={
...
def generate_compounds_cards(active_page: int, data: dict[str, Any], molecule: str | None = None) -> list[dbc.Card]:
    ...
    displayed = data["matching_compounds"][PAGE_SIZE * (active_page - 1):PAGE_SIZE * active_page]
    ...
    for idx, j in enumerate(displayed):
        ...
        if scores_mode:
            ...
            extras.append(dbc.Row(dbc.Col(
                dbc.Progress(key=f"score_{idx}",
        ...
        card = dbc.Card([
            ...
            dbc.CardBody(
                [
                    ...
                    dbc.Button("Compound page", color="primary", href=f"/structure/{j}"),
                    *extras
                ]
            ),
        ],
