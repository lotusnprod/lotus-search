...
from update.download_taxonomy_parenting import run


@patch("update.download_taxonomy_parenting.wd_sparql_to_csv")
...
def test_run_writes_to_file_on_successful_query(mock_open, mock_wd_sparql_to_csv):
    ...
    run(Path("."))
    mock_open.assert_called_once_with(Path(".") / "database_taxo.pkl", "wb")
