...
class TestTaskRunner:
    def test_one_task(self, tmp_path):
        ...
        tasks = [Task(name='task1', f=mock_call.run, params={"foo": "bar"},
    ...
    def test_multiple_tasks_same_group(self, tmp_path):
        ...
        group = Group(name="group1", parallel=False)
        tasks = [
            Task(name='task1', f=mock_call.run, params={"foo": "bar"},
                 group=group),
            Task(name='task2', f=mock_call.run, params={"foo": "bim"},
                 group=group),
        ]
        runner = TaskRunner(tasks, tmp_path)
        runner.run_tasks(parallel=True)
    ...
    def test_parallelism(self, tmp_path):
        ...
        tasks = [
            Task(name='task1', f=f1, params={"foo": "bar"},
                 group=group1),
            Task(name='task2', f=f2, params={"foo": "bim"},
                 group=group2),
        ]
        runner = TaskRunner(tasks, tmp_path)
        runner.run_tasks(parallel=True)
