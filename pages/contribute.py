...
def submit(n_clicks, email, structure, organism, selected_organism, reference):
    if dash.ctx.triggered_id == "submit_contribution_single":
        ...
        if len(structure) > 8192:
            return dbc.Alert(
                "Your molecule is too big, we do not handle molecules this size yet.",
                color="danger",
            )
        elif len(organism) > 100 or len(selected_organism) > 100:
            return dbc.Alert(
                "Your organism name is too long, please use a shorter name.",
                color="danger",
            )
        elif len(reference) > 512:
            return dbc.Alert(
                "Your reference is way too long, please use a doi or a short reference.",
                color="danger",
            )
        elif len(email) > 64 or ("@" not in email and email != ""):
            return dbc.Alert(
                f"Your email {email} is invalid, please use a valid email address or none at all",
                color="danger",
            )
        ...
        with open(f"contributions/{time.time()}_{uuid.uuid4()}.json", "w") as f:
            f.write(js)
        return dbc.Alert(
            "Thank you for your contribution, we will get back to you as soon as possible.",
            color="success",
        )
    return None
...
def update_output(email, content, name):
    if content is not None:
        if len(content) > 1024 * 1024:
            return dbc.Alert(
                f"Sorry your file {name} is too big, we do not handle files this size yet.",
                color="danger",
            )
